...
Texture1D <float2> g_StrandCoordinates : register( t1 );
				...
				const OutputPatch<HS_OUTPUT, OUTPUT_PATCH_SIZE> cPoint )
{
	...
	uint readPos2;
	uint readPos3;
	//if( ( PatchID + 1 ) >= (uint) g_numberOfGuideStrands )
	//{
	//	readPos2 = (uint) ( (PatchID-1) * (totalStrandVertices) + (iVertexHair));
	//	readPos3 = (uint) ( (PatchID-2) * (totalStrandVertices) + (iVertexHair));
	//}
	//else if( ( PatchID + 2 ) >= (uint) g_numberOfGuideStrands )
	//{
	//	readPos2 = (uint) ( (PatchID-1) * (totalStrandVertices) + (iVertexHair));
	//	readPos3 = (uint) ( (PatchID+1) * (totalStrandVertices) + (iVertexHair));
	//}
	//else
	//{
		readPos2 = (uint) ( (PatchID+1) * (totalStrandVertices) + (iVertexHair));
