...
void main (void)
{
	...
	ivec2 size = textureSize(rtGradients, 0);
	...
	for(int i = -w1; i <= w1; ++i)
	{
		...
		sum1 += weight1 * texture2D(rtGradients, vec2(texCoord.x + float(i) * ux, texCoord.y)).xy;
		weights1 += weight1;
	}
	sum1 /= weights1;
	...
	for(int i = -w2; i <= w2; ++i)
	{
		...
		sum2 += weight2 * texture2D(rtGradients, vec2(texCoord.x + float(i) * ux, texCoord.y)).xy;
		weights2 += weight2;
	}
	sum2 /= weights2;
