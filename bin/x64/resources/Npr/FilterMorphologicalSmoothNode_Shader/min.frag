...
void main (void)
{
	...
	if (domainRadius >= 1.0)
	{
		for (int x = -int(domainRadius); x <= int(domainRadius); x++)
		{
			...
			{
				vec4 domainValue = texture2D(inputMap, texCoord.xy + vec2(float(x) * pixelSize.x, float(y) * pixelSize.y));
				
				if (x*x + y*y < int(domainRadius * domainRadius))
				{
					minValue = min(domainValue, minValue);	
				}
			}
		}

		fragColor.rgba = minValue;
	}
	else
	{
		fragColor.rgba = texture2D(inputMap, texCoord.xy);
	}
}
