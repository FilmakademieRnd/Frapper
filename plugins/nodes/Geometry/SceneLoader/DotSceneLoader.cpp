...
{
    std::vector<String>::iterator attributeIterator = userDataAttributes.begin();

    while( attributeIterator != userDataAttributes.end())
    {
        String key = *attributeIterator;
        String value = getAttrib(XMLNode, key, "");
    
        if( value != "")
            pParent->getUserObjectBindings().setUserAny(key, Any(value));
...
void DotSceneLoader::processNode(TiXmlElement *XMLNode, SceneNode *pParent)
{
    ...
    // add ogre container to scene node
    //Frapper::OgreContainer *sceneNodeContainer = new Frapper::OgreContainer(pNode);
    //pNode->setUserAny( Any( sceneNodeContainer));
