...
private slots: //
	...
	//! Slot which is called when one of the flip values changes
	void FlipValueChanged();

	//! Read retarget data from file
	bool readRetargetSetup();

	//! Write retarget data to file
	void writeRetargetSetup();

	//! Slot which is called when the filename changes
	void RetargetDataFilenameChanged();

	//! Reset the retargeting data to default values
	void resetBoneRetargetData();

	//! Updates the null position of the charater by current position of hip center
	void updateZeroPosition();

	//! Updates the scale value for the hip center position
	void updatePositionScale();

	//! reset all bone data
	void resetAvateeringData();
...
private: // data
	...
	//!
	//! The currently selected scale for the hip center joint
	//!
	float m_boneScale;

	//!
	//! The file name to store the retarget data in
	//!
	QString m_retargetDataFilename;

	//!
	//! The Zero position of the characters hip center
	//!
	Ogre::Vector3 m_CharacterZeroPosition;
};
